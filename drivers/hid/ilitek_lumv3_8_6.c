...
static void ilitek_input_report(struct input_dev*, struct touch_data*, int tpnum);
...
#endif
static int hid_suspend(struct hid_device *hdev, pm_message_t message);
static int hid_resume(struct hid_device *hdev);
static int hid_read_keyinfomation(void);
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void ilitek_hid_early_suspend(struct early_suspend *h)
{
	hid_suspend(ilitek.hid.hid_dev, PMSG_SUSPEND);
	printk("%s\n", __func__);
}

static void ilitek_hid_late_resume(struct early_suspend *h)
{
	hid_resume(ilitek.hid.hid_dev);
	printk("%s\n", __func__);
}
#endif
...
{
	uint8_t buf[128] = {0};
	int i, ret = 0, key, flag = 0, ii;
	#ifdef SET_KEY 
	ilitek.hid.keycount = KEY_COUNT;
	printk("%s, key number = %d\n", __func__,ilitek.hid.keycount);
	ilitek.hid.key_xlen = KEY_XLEN;
	ilitek.hid.key_ylen = KEY_YLEN;
	printk(ILITEK_DEBUG_LEVEL "%s, key_xlen: %d, key_ylen: %d\n", __func__, ilitek.hid.key_xlen, ilitek.hid.key_ylen);	
	#else
	buf[0] = 0x03;
	buf[1] = 0xa3;
	buf[2] = 0x01;
	buf[3] = 0x10;
	buf[4] = 0x20;
	ret = ilitek_hid_set_report(0x02, 0x03, buf, 0x40);
	if(!ret){
		printk("%s, read key number, failed\n", __func__);
		return ret;
	}
	else{
		msleep(10);
		ilitek.hid.keycount = ilitek.cmd.field->value[11];
		printk("%s, key number = %d,%d\n", __func__,ilitek.hid.keycount,ilitek.cmd.field->value[11]);
	}
	if(ilitek.hid.keycount)
	{
		buf[0] = 0x03;
		buf[1] = 0xa3;
		buf[2] = 0x01;
		buf[3] = (ilitek.hid.keycount*5+4);
		buf[4] = 0x22;
		for(ii = 0; ii < 10; ii++){
			ret = ilitek_hid_set_report(0x02, 0x03, buf, 0x40);
			if(!ret){
				printk("%s, read jey information, failed\n", __func__);
				return ret;
			}
			msleep(10*(ii+1));
			for(i = 0; i < buf[3]; i++){
				if(ilitek.cmd.field->value[i+3]==0xff){
					printk("%s, value = %d, failed\n", __func__,ilitek.cmd.field->value[i+3]);
					flag = 1;
					break;
				}
				if(flag == 0)
		...
		ilitek.hid.key_xlen = (ilitek.cmd.field->value[3] << 8) + ilitek.cmd.field->value[4];
		ilitek.hid.key_ylen = (ilitek.cmd.field->value[5] << 8) + ilitek.cmd.field->value[6];
		printk(ILITEK_DEBUG_LEVEL "%s, key_xlen: %d, key_ylen: %d\n", __func__, ilitek.hid.key_xlen, ilitek.hid.key_ylen);
	}
	#endif
	//print key information
	for(i = 0; i < ilitek.hid.keycount; i++)
	{
	#ifndef SET_KEY 
		ilitek.hid.keyinfo[i].id = ilitek.cmd.field->value[i*5+7];	
		ilitek.hid.keyinfo[i].x = (ilitek.cmd.field->value[i*5+8] << 8) + ilitek.cmd.field->value[i*5+9];
		ilitek.hid.keyinfo[i].y = (ilitek.cmd.field->value[i*5+10] << 8) + ilitek.cmd.field->value[i*5+11];
	#else
		ilitek.hid.keyinfo[i].id = key_id[i];
		ilitek.hid.keyinfo[i].x = keyinfo_x[i];
		ilitek.hid.keyinfo[i].y = keyinfo_y[i];
	#endif
		ilitek.hid.keyinfo[i].status = 0;
		printk(ILITEK_DEBUG_LEVEL "%s, key_id: %d, key_x: %d, key_y: %d, key_status: %d\n", __func__, ilitek.hid.keyinfo[i].id, ilitek.hid.keyinfo[i].x, ilitek.hid.keyinfo[i].y, ilitek.hid.keyinfo[i].status);
	}
	if(ilitek.hid.keycount){
		for(key = 0; key < ilitek.hid.keycount; key++)
		{
			printk("%s,register key :%d\n",__func__,ilitek.hid.keyinfo[key].id );
			set_bit(ilitek.hid.keyinfo[key].id & KEY_MAX, ilitek.hid.input_dev->keybit);
		}
	}
	return 0;
}
...
static int ilitek_hid_set_report(unsigned char type, unsigned char id, void *buf, int size)
{
	//printk("USB_REQ_SET_CONFIGURATION = 0x%2x,data2 = 0x%2x,data3 = 0x%2x,time = %d\n", USB_REQ_SET_CONFIGURATION, USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_DIR_OUT, (type << 8) + id,USB_CTRL_SET_TIMEOUT);
	return usb_control_msg(hid_to_usb_dev(ilitek.cmd.hid_dev), usb_sndctrlpipe(hid_to_usb_dev(ilitek.cmd.hid_dev), 0),
		USB_REQ_SET_CONFIGURATION,
		USB_TYPE_CLASS | USB_RECIP_INTERFACE | USB_DIR_OUT,
		(type << 8) + id, 1, buf, size, USB_CTRL_SET_TIMEOUT);
}
...
static ssize_t ilitek_file_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos)
{
	...
	else if(strcmp(buffer, "fwinfo") == 0)
	{
		if(ilitek.hid.support_hid_device)
		{
			if(ilitek.hid.valid_hid_register)
			{
				if(ilitek.hid.hid_dev)
				{
					...
					ret = ilitek_hid_set_report(0x02, 0x03, buf, 0x40);
					if(!ret){
						printk("%s, read FW version, failed\n", __func__);
					}
					else{
						msleep(10);
						printk("%s, FW version:%d.%d.%d\n", __func__,ilitek.cmd.field->value[3],ilitek.cmd.field->value[4],ilitek.cmd.field->value[5]);
					}
					//for(i = 0; i<64; i++)
					//{
					//	printk("value[%d]=0x%x,",i,ilitek.cmd.field->value[i]);
					//}
					hid_read_keyinfomation();
				}
			}
		}	
	}
	else if(strcmp(buffer, "keyinfo") == 0)
	{
		if(ilitek.hid.support_hid_device)
		{
			if(ilitek.hid.valid_hid_register)
			{
				if(ilitek.cmd.hid_dev)
				{
					hid_read_keyinfomation();
				}
			}
...
static void hid_feature_mapping(struct hid_device *hdev,struct hid_field *field, struct hid_usage *usage)
{
	switch (usage->hid) 
	{
		case HID_DG_INPUTMODE:
			ilitek.hid.inputmode = field->report->id;
			break;
		...
		case HID_UP_DIGITIZER:
			switch(usage->hid)
			{
				case HID_DG_TIPSWITCH:
					//printk("%s,%s,%d,hid_input_mapping,HID_DG_TIPSWITCH\n",__FILE__,__func__,__LINE__);
				...
				case HID_DG_CONTACTID:
					//printk("%s,%s,%d,hid_input_mapping,HID_DG_CONTACTID\n",__FILE__,__func__,__LINE__);
				...
				case HID_DG_TIPPRESSURE:
					//printk("%s,%s,%d,hid_input_mapping,HID_DG_TIPPRESSURE\n",__FILE__,__func__,__LINE__);
...
static int hid_probe(struct hid_device *hdev, const struct hid_device_id *dev_id)
{
	...
	if(!ilitek.dev.remove_flag){
		...
		ilitek.hid.early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
		ilitek.hid.early_suspend.suspend = ilitek_hid_early_suspend;
		ilitek.hid.early_suspend.resume = ilitek_hid_late_resume;
		register_early_suspend(&ilitek.hid.early_suspend);
	#endif
	}
...
static void hid_remove(struct hid_device *hdev)
{
	...
	if(ilitek.dev.remove_flag){
		...
		#ifdef CONFIG_HAS_EARLYSUSPEND
		unregister_early_suspend(&ilitek.hid.early_suspend);
		#endif
...
static void ilitek_input_report(struct input_dev *input_dev,struct touch_data *tp, int tpnum)
{
    ...
    if(ilitek.hid.support_touch_point == 1) 
	{
        if(tp[0].valid) 
		{
            input_event(ilitek.hid.input_dev, EV_ABS, ABS_X, tp[0].x);
            input_event(ilitek.hid.input_dev, EV_ABS, ABS_Y, tp[0].y);
            input_event(ilitek.hid.input_dev, EV_KEY, BTN_TOUCH, 1);
        }
        else 
		{
            input_event(ilitek.hid.input_dev, EV_KEY, BTN_TOUCH, 0);
        }
    }
    else {
        for(i = 0; i < tpnum; i++) 
		{
            input_mt_slot(ilitek.hid.input_dev, tp[i].id);
            if(tp[i].valid) 
			{
				if(ilitek.hid.keyflag == 0)
				{
					for(j = 0; j <= ilitek.hid.keycount; j++)
					{
						if((tp[i].x >= ilitek.hid.keyinfo[j].x && tp[i].x <= ilitek.hid.keyinfo[j].x + ilitek.hid.key_xlen) && (tp[i].y >= ilitek.hid.keyinfo[j].y && tp[i].y <= ilitek.hid.keyinfo[j].y + ilitek.hid.key_ylen))
						{
							input_report_key(ilitek.hid.input_dev,  ilitek.hid.keyinfo[j].id, 1);
							ilitek.hid.keyinfo[j].status = 1;
							ilitek.hid.touch_key_hold_press = 1;
							DBG("Key, Keydown ID=%d, X=%d, Y=%d, key_status=%d,keyflag=%d\n", ilitek.hid.keyinfo[j].id ,tp[i].x ,tp[i].y , ilitek.hid.keyinfo[j].status,ilitek.hid.keyflag);
							break;
						}
					}
				}
				if(ilitek.hid.touch_key_hold_press == 0)
				{
					input_mt_report_slot_state(ilitek.hid.input_dev, MT_TOOL_FINGER, true);
					input_event(ilitek.hid.input_dev, EV_ABS, ABS_MT_POSITION_X, tp[i].x);
					input_event(ilitek.hid.input_dev, EV_ABS, ABS_MT_POSITION_Y, tp[i].y);
					input_event(ilitek.hid.input_dev, EV_ABS, ABS_MT_TOUCH_MAJOR, 1);
					DBG("touch down,x%d=%d,y%d=%d\n", tp[i].id, tp[i].x, tp[i].id, tp[i].y);
					ilitek.hid.keyflag = 1;
				}
				if(ilitek.hid.touch_key_hold_press == 1)
				{
					for(j = 0; j <= ilitek.hid.keycount; j++)
					{
						if((ilitek.hid.keyinfo[j].status == 1) && (tp[i].x < ilitek.hid.keyinfo[j].x || tp[i].x > ilitek.hid.keyinfo[j].x + ilitek.hid.key_xlen 
						|| tp[i].y < ilitek.hid.keyinfo[j].y || tp[i].y > ilitek.hid.keyinfo[j].y + ilitek.hid.key_ylen))
						{
							input_report_key(ilitek.hid.input_dev,  ilitek.hid.keyinfo[j].id, 0);
							ilitek.hid.keyinfo[j].status = 0;
							ilitek.hid.touch_key_hold_press = 0;
							DBG("Key, Keyup ID=%d, X=%d, Y=%d, key_status=%d\n", ilitek.hid.keyinfo[j].id, tp[i].x , tp[i].y, ilitek.hid.keyinfo[j].status);
							break;
						}
					}
				}
			...
			{
				ilitek.hid.keyflag = 0;
				if(ilitek.hid.touch_key_hold_press == 1)
				{
					for(i = 0; i < ilitek.hid.keycount; i++)
					{
						if(ilitek.hid.keyinfo[i].status)
						{
							input_report_key(ilitek.hid.input_dev, ilitek.hid.keyinfo[i].id, 0);
							ilitek.hid.keyinfo[i].status = 0;
							ilitek.hid.touch_key_hold_press = 0;
							DBG("Key, Keyup ID=%d, X=%d, Y=%d, key_status=%d, touch_key_hold_press=%d\n", ilitek.hid.keyinfo[i].id , tp[i].x , tp[i].y , ilitek.hid.keyinfo[i].status, ilitek.hid.touch_key_hold_press);
						}
					}
				}
                input_mt_report_slot_state(ilitek.hid.input_dev, MT_TOOL_FINGER, false);
                DBG("touch up ,x%d=%d,y%d=%d\n", tp[i].id, tp[i].x, tp[i].id, tp[i].y);
            }
        }
    }
    input_mt_report_pointer_emulation(ilitek.hid.input_dev, true);
    input_sync(ilitek.hid.input_dev);
}
#endif
